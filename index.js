...
(1) Name of the first artist (0th index) in the array (2) Bio of the third artist (2nd index) in the array */
...
/* Task 3: Create a function called `getArtistByIndex` that takes two arguments: (1) artists array
(2) a number which is the desired index in the array. getArtistByIndex returns a string in the format `The artist at index {id} is {name}.` For example, if getArtistByIndex is invoked with the artists dataset and the number 0, it will return `The artist at index 0 is Amedeo Modigliani`. */
...
function get20s(artists){
  ...
  for (i = 0; i < artists.length; i++) {
    birth = artists[i].years.slice(0, 4);
    if (birth >= 1900 && birth < 2000) {
      newArray.push({name: artists[i].name, years: artists[i].years});
    }
    else if (birth < 1900 && artists[i].years.includes("- 19")) {
      newArray.push({name: artists[i].name, years: artists[i].years});
    }
  }
...
/* Task 5: Create a function called `removeArtist` that takes two arguments: (1) artists array (2) a number which is the desired index in the array. removeArtist removes an artist from the array at the index and console.logs the length of the remaining dataset. For example, if removeArtist is invoked with the data and the number 0, it will remove Amedeo Modigliani from our dataset and log the number 19. Note that sucessfully invoking this function multiple times without refreshing your browser will continuously remove artists from the array until there are none left. If you refresh your browser, the data will reset. */
function removeArtist(artists, id) {
  artists.splice(id, 1);
  console.log(artists.length);
}

/* Task 6: Create a function called `addArtist` that can accept an object of information and add it to the artists array. Then, add a 21st artist to the array (you) with custom information! For example, you could add the following information: id: 20, name: Your Name Here, years: Your Birth Year - current day, genre: Web Design, nationality: Your Nationality Here bio: Add 1-2 sentences (or use lorem ipsum) At the end, this function should return the new array with information added"*/
me = {
  id: "20",
  name: "Lena Harris",
  years: "1988 - Present",
  ...
  bio: "日本人ならそんなことはけっしてしないでしょう。 ８ギガバイトのハードディスクがあれば十分だと思います。 ９時半の電車に乗れば、何時に秋田につきますか。 ここで写真を撮らないでください。 エヴェレスト山は海抜２９、００２フィートです。 ５月１０日月曜日の午後３時に東京駅八重洲中央口で待ち合わせをしていただけませんか。 「これはとてもおもしろそうだね」とひろしが言います。 １１時になっているよ。 肩慣らしには丁度いいかも。 エコロジーのために堪え忍ぶのではなく、自然と調和した住環境の快適性が必要である。",
}
...
/* Task 7: Create a function called lotsOfArt() that takes one argument: (1) artists array and returns an array with names of artists who painted more than 100 paintings. For example lotsOfArt(artists); will return ["Amedeo Modigliani", "Rene Magritte", ..."Albrecht Dürer"]*/
function lotsOfArt(artists){
  newArray = [];
  for (i = 0; i < artists.length; i++) {
    if (artists[i].paintings > 100) {
      newArray.push({name: artists[i].name, paintings: artists[i].paintings})
    }
  }
  console.log(newArray);
}
...
// 🎨🎨 STRETCH 🎨🎨//
/* STRETCH 1: Programtically console.log HTML element structure. In HTML, every artist and associated content uses the following structure: <div id="artist"> <div class="image"> <img src="https://images.fineartamerica.com/images/artworkimages/mediumlarge/3/starry-night-by-vincent-van-gogh-vincent-van-gogh.jpg"/> </div> <div class = "name"> <a href="https://en.wikipedia.org/wiki/Vincent_van_Gogh"> Vincent Van Gogh</a> </div> <div class = "bio">Vincent Willem van Gogh (Dutch: [ˈvɪnsɛnt ˈʋɪləm vɑŋ ˈɣɔx] (listen); 30 March 1853 – 29 July 1890) was a Dutch Post-Impressionist painter who is among the most famous and influential figures in the history of Western art. In just over a decade he created about 2,100 artworks, including around 860 oil paintings, most of them in the last two years of his life. They include landscapes, still lifes, portraits and self-portraits, and are characterised by bold colours and dramatic, impulsive and expressive brushwork that contributed to the foundations of modern art. However, he was not commercially successful, and his suicide at 37 followed years of mental illness and poverty.</div> </div> Create a function called `getHTML()` that takes the parameter `data` and uses a for..in loop to format and console.log data like the example above. The function should console.log 50 chunks of HTML code that match the structure above. ‼️ You do **NOT** need to get these to display on your page, but you can copy and paste the result into your HTML file if you'd like to see what that would look like. */
...
function randomize(artists){
  newArray = [artists.length];
  for (i = 0; i < artists.length; i++) {
    random = Math.floor(Math.random() * artists.length) + 1;
    if (typeof newArray[random] === 'undefined') {
      newArray[random] = artists[i];
    }
  }
  console.log(newArray);
}
